 ...
 * for every tool in `agent/tools/` (implementation plan §2.4).
 *
 * `lib/api/respond.ts` is the same idea for the HTTP front door, and its own
 * doc comment says outright that Phase 4's tools need their own, differently
 * worded mapping of the same two action-layer errors — "the model for it, not
 * a shared dependency of it". This module is that mapping. It keeps
...
/**
 * What every tool returns. One shape means `agent/instructions.md` can teach
 * the model a single rule for reading any tool result, and Phase 5's chat UI
 * has exactly one thing to render.
 */
export type ToolResult<T> =
  { ok: true; data: T } | { ok: false; kind: ToolFailureKind; message: string }
